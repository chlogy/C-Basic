...
int main(void)
{
	...
	while (!feof(fp))
	{
		fscanf_s(fp, "%d%d%d", &temp1, &temp2, &temp3);
		m_row++;
	}
	...
	for (int i = 0; i < m_row; i++)					// 행을 증가 시킴
	{
		for (int j = 0; j < m_col; j++)				// 열을 증가 시킴
		{
			fscanf_s(fp, "%d", &matrix[i][j]);		// 행열의 값을 파일에서 하나씩 쓰는것
		}
	}
	fclose(fp);
	...
	for (int i = 0; i < m_row; i++)  free(matrix[i]);
	free(matrix);
